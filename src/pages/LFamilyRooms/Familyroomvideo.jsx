...
// Card Component
const FamilyRoomVideoCard = ({ name, review, video }) => {
  return (
...
const FamilyRoomVideoSection = () => {
  ...
  useEffect(() => {
    const fetchVideos = async () => {
      try {
        setLoading(true);
        const response = await axiosInstance.get("/family-room-videos");
    ...
    <section className="py-12 px-4 text-center bg-gray-100">
      ...
      <div className="flex flex-wrap justify-center gap-6">
        {videos.map((video, index) => (
          <FamilyRoomVideoCard key={index} {...video} />
        ))}
      </div>
    </section>
  );
...
export default FamilyRoomVideoSection;
